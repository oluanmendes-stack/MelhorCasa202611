...
import { ArrowLeft, PiggyBank, Target, Calendar, TrendingUp, Plus, Home, MapPin, Car, Maximize2, Trash2 } from "lucide-react";
import { Link } from "react-router-dom";
import { toast } from "sonner";
import { getCurrentUser, removeLikeForUser } from "@/lib/auth";
...
export default function Cofrinho() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (user) {
        setLikedProperties(user.likedProperties as Property[]);
      } else {
  ...
  const calculateDownPayment = (valorStr: string): number => {
    ...
    return valor * 0.2; // 20% down payment
  };

  const calculateITBI = (valorStr: string): number => {
    const valor = parseNumericValue(valorStr);
    return valor * 0.05; // 5% ITBI
  };

  const calculateDocumentation = (valorStr: string): number => {
    const valor = parseNumericValue(valorStr);
    return valor * 0.01; // 1% documentation
  };

  const calculateTotalUpfront = (valorStr: string): number => {
    return calculateDownPayment(valorStr) + calculateITBI(valorStr) + calculateDocumentation(valorStr);
  };

  const calculateRequiredPropertyValue = (valorStr: string): number | null => {
    const totalNeeded = calculateTotalUpfront(valorStr);

    // Se o usuário já tem o suficiente, não precisa recalcular
    if (totalSavings >= totalNeeded) {
      return null;
    }

    // Se não tem o suficiente, o imóvel precisa ser avaliado por um valor maior
    // para que a diferença faltante seja adicionada ao valor total
    const difference = totalNeeded - totalSavings;
    const valor = parseNumericValue(valorStr);
    const newValue = valor + difference;
  ...
  const handleDeleteLike = async (propertyId: string) => {
    ...
    // Remover também das metas
    setSavingsGoals(prev => prev.filter(g => g.propertyId !== propertyId));

    const user = await getCurrentUser();
    try {
      if (user) {
        // Se há usuário logado, remover do banco de dados
        await removeLikeForUser(user.id, property);
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <div className="border-t border-gray-200 pt-2">
                      ...
                      <div className="flex justify-between text-sm mb-2">
                        <span className="text-gray-600">ITBI (5%):</span>
                        <span className="font-semibold text-amber-600">
                          R$ {calculateITBI(property.valor).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm mb-2">
                        <span className="text-gray-600">Documentação (1%):</span>
                        <span className="font-semibold text-amber-600">
                          R$ {calculateDocumentation(property.valor).toLocaleString()}
                        </span>
                      </div>
                    </div>
                    <div className="bg-red-50 border border-red-200 rounded p-3">
                      <div className="flex justify-between text-sm">
                        <span className="text-red-900 font-semibold">Total à Vista:</span>
                        <span className="font-bold text-red-700">
                          R$ {calculateTotalUpfront(property.valor).toLocaleString()}
                        </span>
                      </div>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Você já tem:</span>
                      <span className="font-bold text-green-600">
                        R$ {totalSavings.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Ainda precisa:</span>
                      <span className="font-bold text-red-600">
                        R$ {Math.max(0, calculateTotalUpfront(property.valor) - totalSavings).toLocaleString()}
                      </span>
                    </div>
                  </div>

                  {/* Required Property Value Calculator */}
                  {calculateRequiredPropertyValue(property.valor) && (
                    <div className="bg-yellow-50 border border-yellow-300 rounded p-3 mb-4">
                      <p className="text-xs text-yellow-700 mb-2">
                        Com o valor que você tem disponível, o imóvel pode ser avaliado por:
                      </p>
                      <div className="text-lg font-bold text-yellow-900">
                        R$ {calculateRequiredPropertyValue(property.valor)?.toLocaleString()}
                      </div>
                      <p className="text-xs text-yellow-700 mt-1">
                        *(Estratégia do corretor: aumentar a avaliação do imóvel para adicionar a diferença ao financiamento)
                      </p>
                    </div>
                  )}
